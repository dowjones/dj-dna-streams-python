...
class Listener(object):
    ...
    def listen(
        self,
        on_message_callback,
        maximum_messages=DEFAULT_UNLIMITED_MESSAGES,
        subscription_id="",
        batch_size=10,
    ):
        ...
        while maximum_messages is None or count < maximum_messages:
            try:
                ...
                if results:
                    if len(results.received_messages) > 0:
                        for message in results.received_messages:
                            ...
                            logger.info(
                                "Received news message with ID: {}".format(
                ...
                if isinstance(e, NotFound):
                    raise e
                logger.error(
                    "Encountered a problem while trying to pull a message from a stream. Error is as follows: {}".format(
                        str(e)
                    )
                )
                logger.error(
                    "Due to the previous error, system will pause 10 seconds. System will then attempt to pull the message from "
                    "the stream again."
                )
                time.sleep(10)
                pubsub_client = pubsub_service.get_client(self.config)
    ...
    def listen_async(self, on_message_callback, subscription_id=""):
        def ack_message_and_callback(message):
            ...
            logger.info(
                "Received news message with ID: {}".format(pubsub_msg["data"][0]["id"])
            )
            news_msg = pubsub_msg["data"][0]["attributes"]
    ...
    def listen_async_ha(self, on_message_callback):
        ...
        main_subscription_id = (
            subscription_id
            if "-bak-" not in subscription_id
            else subscription_id.replace("-filtered-bak-", "-filtered-")
        )
        backup_subscription_id = (
            subscription_id
            if "-bak-" in subscription_id
            else subscription_id.replace("-filtered-", "-filtered-bak-")
        )
