...
if (
    ...
    and gcp_pubsub_topic is not None
    and gcp_creds is not None
):
    listener = Listener()
        ...
        try:
            if "action" in factiva_message.keys():
                ...
                if factiva_message["action"] == "add":
                    ...
                    print_message(
                        f"[ARTICLE] [ADD] AN: {factiva_message['an']} - {factiva_message['title']}"
                    )
                elif factiva_message["action"] == "rep":
                    # Upsert/Update/AddNewVersion the article in the repository. Handle repeated messages.
                    print_message(
                        f"[ARTICLE] [REP] AN: {factiva_message['an']} - {factiva_message['title']}"
                    )
                elif factiva_message["action"] == "del":
                    # Delete or mark as deleted the article in the repository. Handle inexistent article AN.
                    print_message(
                        f"[ARTICLE] [DEL] AN: {factiva_message['an']} - *** DELETE ***"
                    )
                else:
                    print_message(
                        f"Factiva Action Not Handled: {factiva_message['action']}"
                    )

                logger.info(
                    f"Sent to GCP Pub/Sub, message with AN: {factiva_message.get('an', 'N/A')}"
                )
                ...
                if factiva_message["event_type"] == "source_delete":
                    ...
                    print_message(
                        f"[EVENT] [SOURCE_DELETE] Source: {factiva_message['source_code'].upper()} - {factiva_message['description']}"
                    )
                else:
                    print_message(
                        f"Factiva Event Type Not Handled: {factiva_message['event_type']}"
                    )

                logger.info(
                    f"Sent to GCP Pub/Sub, message with event: {factiva_message.get('event_type', 'N/A')}"
                )
