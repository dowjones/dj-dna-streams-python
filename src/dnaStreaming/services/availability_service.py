...
def get_active_region(api_host, subscription_id, user_key):
    ...
    try:
        ...
        headers = {"subscription-id": subscription_id, "user-key": user_key}
...
def ha_listen(
    ...
    while not stop_event.is_set():
        ...
        if active_region is None or active_region not in (MAIN_REGION, BACKUP_REGION):
            logger.warning(
                f"Got invalid region from API: {active_region}. Listener will keep reading from region {current_region}"
            )
            continue

        if current_region != active_region:
            logger.warning(
                f"Switch event detected in, switching from region {current_region} to region {active_region}"
            )
            ...
            if active_region == MAIN_REGION:
                streaming_pull_future = main_pubsub_client.subscribe(
                    main_subscription_path,
                    callback=wrapped_callback(main_subscription_path),
                )
            else:  # active_region == BACKUP_REGION
                streaming_pull_future = backup_pubsub_client.subscribe(
                    backup_subscription_path,
                    callback=wrapped_callback(backup_subscription_path),
                )
